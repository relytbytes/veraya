...
export async function GET(req: NextRequest) {
  ...
  const [
    ...
    prisma.order.aggregate({
      where: { status: "COMPLETED", createdAt: { gte: yestStart, lte: yestHourEnd } },
      _sum: { total: true },
    }),
    prisma.order.aggregate({
      where: { status: "COMPLETED", createdAt: { gte: lwStart, lte: lwHourEnd } },
      _sum: { total: true },
    }),
  ...
  const diag = buildDiagnosis({
    ...
    outOfStockNames: outOfStock.map((x) => x.ingredient.name),
    lowStockNames: lowStock.map((x) => x.ingredient.name),
    active86Names: active86.map((e) => e.menuItem.name),
