...
import { View, Text, TouchableOpacity, RefreshControl, ActivityIndicator, Alert, Animated, useWindowDimensions } from "react-native";
...
function urgencyAccent(dateStr: string): string {
  const secs = Math.floor((Date.now() - new Date(dateStr).getTime()) / 1000);
  if (secs < 180) return C.jade;
  if (secs < 360) return C.ember;
  return C.coral;
}
...
export default function BarScreen() {
  ...
  async function bump(orderId: string) {
    setBumping(orderId);
    try {
      await kitchenAction({ orderId, action: "bump", station: "BAR" });
      qc.setQueryData<Order[]>(["bar"], (prev) => prev?.filter((o) => o.id !== orderId) ?? []);
    } catch (e: unknown) {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: C.void }}>
      <CollapsingHeader
        ...
        scrollY={scrollY}
        left={<Ionicons name="wine-outline" size={24} color={C.gold} />}
      />
      ...
      {isLoading ? (
        ...
        >
          <View style={{ height: 80, width: 80, borderRadius: 24, backgroundColor: T.jade, alignItems: "center", justifyContent: "center", borderWidth: 1, borderColor: C.rim }}>
            <Ionicons name="wine" size={40} color={C.jade} />
          </View>
          <Text style={{ color: C.pearl, fontSize: 18, fontWeight: "600" }}>All caught up!</Text>
          <Text style={{ color: C.mist, fontSize: 14 }}>No drink tickets</Text>
          ...
          {orders.map((order) => {
            ...
            return (
              <View
                ...
                style={{
                  width: cellW,
                  backgroundColor: C.surface, borderRadius: 18, borderWidth: 1, borderColor: C.rim,
                  borderLeftWidth: 4, borderLeftColor: accent, overflow: "hidden", ...shadow.md,
                }}
              >
                <View style={{ backgroundColor: C.surfaceHi, borderBottomWidth: 1, borderBottomColor: C.rim, paddingHorizontal: 16, paddingVertical: 12, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                  <Text style={{ color: C.pearl, fontWeight: "700", fontSize: 17 }}>
                    {order.table ? `Table ${order.table.number}` : order.type}
                  </Text>
                  <View style={{ backgroundColor: C.surfaceHov, borderRadius: 99, paddingHorizontal: 10, paddingVertical: 3, borderWidth: 1, borderColor: C.rim, flexDirection: "row", alignItems: "center", gap: 4 }}>
                    <Ionicons name="time-outline" size={12} color={C.mist} />
                    <Text style={{ color: C.mist, fontFamily: "monospace", fontSize: 13 }}>{elapsed(order.createdAt)}</Text>
                  </View>
                </View>
                ...
                <View style={{ paddingHorizontal: 12, paddingBottom: 12 }}>
                  <TouchableOpacity
                    ...
                    style={{
                      ...
                      flexDirection: "row", gap: 6, backgroundColor: allDone ? C.gold : C.ember, ...(allDone ? shadow.gold : {}),
                    }}
                  >
                    {bumping === order.id ? (
                      ...
                      <>
                        <Ionicons name={allDone ? "checkmark-circle" : "arrow-up-circle-outline"} size={16} color={allDone ? C.void : "#fff"} />
                        <Text style={{ fontWeight: "700", fontSize: 14, color: allDone ? C.void : "#fff" }}>
